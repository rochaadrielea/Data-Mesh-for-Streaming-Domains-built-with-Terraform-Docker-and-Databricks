...
resource "null_resource" "build_and_push_docker" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      docker tag railsimulator:latest railsightregistry1.azurecr.io/railsimulator:latest
      az acr login --name railsightregistry1
      docker push railsightregistry1.azurecr.io/railsimulator:latest
    EOT
  }
}
